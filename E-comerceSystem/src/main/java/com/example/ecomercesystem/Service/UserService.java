...
public class UserService {
    ...
    public boolean userBuyProduct(Integer productId,Integer UserId,Integer merchantId) {
        User user = null;
        Integer index=0;
        for (int i = 0; i < users.size(); i++) {
        ...
        for(int i=0;i<merchantStockService.getall().size();i++)
        {
            if(merchantStockService.getall().get(i).getId()==merchantId)
            {
                merchantStockService.getall().get(i).setStock(merchantStockService.getall().get(i).getStock()-1);
            }
        }
        return true;
    }
